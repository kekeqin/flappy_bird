...
import socket
import random
from event import Event, PLAYER_LIST, QUIT, READY
...
class GameWindow:
    ...
    def update_window(self):     
        ...
        if self.player.rect.y < 0 or self.player.rect.y > H - self.player.frame_height:
            self.player.dead = True  # 玩家撞击地面
    ...
    def handle_ready_event(self, event):
        self.players_ready = len(self.player_list)
        print(f"ALL players are ready. Ready count: {self.player_list}")
    ...
    def start(self):
        ...
        while self.running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_SPACE and not self.game_started:
                        self.channel.send(data = Event(id=READY, data=None).to_dict())
                        self.game_started = True  # 按下空格键开始游戏
                        if self.game_started and self.players_ready == len(self.player_list):
                            self.game_started = True
                            # 显示等待界面
                            self.window.blit(screen, (0, 0))
                            font = pygame.font.SysFont("Arial", 17)
                            text = font.render("Waiting for other players...", True, (0, 0, 0))
                            self.window.blit(text, (50, 200))
                            pygame.display.update()
                            continue
            ...
            if self.player.rect.y < 0 or self.player.rect.y > H - self.player.frame_height or pygame.sprite.spritecollideany(self.player, self.pipes):
                self.player.dead = True  # 设置小鸟死亡标志

            if not self.game_started:
                # 如果游戏未开始，可以在这里显示一些等待开始的界面或文本
                self.window.blit(screen, (0, 0))  
                self.window.blit(start, (50, 50))
                self.window.blit(floor, (0, H - floor.get_height()))
                font = pygame.font.SysFont("Arial", 17)
                text = font.render("Press SPACE to start", True, (0, 0, 0))
                self.window.blit(text, (50, 200))  # 显示开始游戏的提示
